...
public class TestTask implements IService {
    ...
    public void exec() {
        ...
        Arrays.asList( "/bus 內湖幹線 \"康寧醫院\"", "/movie 哈拉影城", "/mrt 中山 松山", "/station 康寧醫院 捷運內湖站", "/theater 哈拉影城", thsr ).forEach( i -> {
            String[] params = i.split( " ", 2 );

            Utils.call( String.format( COMMAND_URL, token, channel, params[ 0 ], params[ 1 ] ) );
        } );
    }
}